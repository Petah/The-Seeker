///// Calcule l'ensemble des ombres et lumi�res /////

var length, samples, r, g, b, j, sl, rl, i, i1;
...
// Calcul de la puissance de l'�clairage ambiant
...
{
    surface_set_target(sl_layers_surface[i1]); // Cr�ation des masks pour le rendu des layers
    draw_clear(c_white);
...
if sl_sunshadows_refresh && sl_sunshadows_refreshcounter<=0
{
    ...
    if sl_sunshadows_light < 1
    {
        for (i1=0; i1<=sl_layers_count; i1+=1)
        {
            surface_set_target(sl_sunshadows_surface1[0]); // Cr�ation du sample
            draw_clear(c_white);
            ...
            draw_set_blend_mode_ext(bm_src_color,bm_one);
            draw_set_color(c_black);
            draw_rectangle(0,0,(view_wview[global.sl_viewid]+sl_sunshadows_margin*2)*sl_sunshadows_texturesize,(view_hview[global.sl_viewid]+sl_sunshadows_margin*2)*sl_sunshadows_texturesize,0);
            draw_set_color(c_white);
            ...
            draw_surface_ext(sl_sunshadows_surface1[i],-sl_sunshadows_margin+sl_buffer_xmargin,-sl_sunshadows_margin+sl_buffer_ymargin,1/sl_sunshadows_texturesize*sl_buffer_texturesize,1/sl_sunshadows_texturesize*sl_buffer_texturesize,0,c_white,1);
            ...
            for (i2=0; sl_sunshadows_texlist[i2,1]!=-1; i2+=1) if sl_sunshadows_texlist[i2,0] = i1
            {
                ...
                xs    = 1+(sl_sunshadows_length*sl_sunshadows_layerscale[sl_sunshadows_texlist[i2,0]])/sl_sunshadows_texlist[i2,3];
                
                with sl_sunshadows_texlist[i2,1] draw_sprite_ext(other.sl_sunshadows_texlist[other.i2,2],0,x+other.sl_xx,y+other.sl_yy,image_xscale*other.xs,image_yscale,other.sl_sunshadows_direction,c_black,image_alpha);
            }
            for (i2=0; global.sl_texlist_shad[i2,0]!=-1; i2+=1) if global.sl_texlist_shad[i2,0] = i1
            draw_sprite_ext(global.sl_texlist_shad[i2,1],global.sl_texlist_shad[i2,2],global.sl_texlist_shad[i2,3]-view_xview[global.sl_viewid]+sl_buffer_xmargin,global.sl_texlist_shad[i2,4]-view_yview[global.sl_viewid]+sl_buffer_ymargin,global.sl_texlist_shad[i2,5],global.sl_texlist_shad[i2,6],global.sl_texlist_shad[i2,7],c_black,global.sl_texlist_shad[i2,8]);
...
for (i1=0; global.sl_lightlist[i1]!=-1; i1+=1) with global.sl_lightlist[i1]
{
    ...
    sl_sz = global.sl_light_txsize/2;
    ...
    { 
        ...
        if refresh or sl_light_surface = -1
        {
            ...
            if sl_light_castshadow
            {
                surface_set_target(global.sl_light_gbuffer); // Conception du sample
                draw_clear(c_white);
                
                sl_xx = global.sl_light_txsize/sl_light_xscale*(0.5*sl_light_xscale-0.5);
                sl_yy = global.sl_light_txsize/sl_light_yscale*(0.5*sl_light_yscale-0.5);
                sl_w1 = sl_light_x+sl_sz*sl_light_xscale  sl_w2 = sl_light_x-sl_sz*sl_light_xscale;
                sl_h1 = sl_light_y+sl_sz*sl_light_yscale  sl_h2 = sl_light_y-sl_sz*sl_light_yscale;
                ...
                {var shspr if other.sl_light_shadowlist[other.i2,1]>-1 shspr=other.sl_light_shadowlist[other.i2,1] else shspr=sprite_index;
                 draw_sprite_ext(shspr,floor(image_index),(x-other.sl_light_x+other.sl_sz)/other.sl_light_xscale+other.sl_xx,(y-other.sl_light_y+other.sl_sz)/other.sl_light_yscale+other.sl_yy,image_xscale/other.sl_light_xscale,image_yscale/other.sl_light_yscale,image_angle,c_black,image_alpha*other.sl_light_shadowsharpness)};
            
                for (i2=0; global.sl_castlist[i2,0]!=-1; i2+=1) if global.sl_castlist[i2,3] = true
                draw_sprite_ext(global.sl_castlist[i2,4],global.sl_castlist[i2,5],(global.sl_castlist[i2,6]-sl_light_x+sl_sz)/sl_light_xscale+sl_xx,(global.sl_castlist[i2,7]-sl_light_y+sl_sz)/sl_light_yscale+sl_yy,global.sl_castlist[i2,9]/sl_light_xscale,global.sl_castlist[i2,10]/sl_light_yscale,global.sl_castlist[i2,11],c_black,global.sl_castlist[i2,12]*sl_light_shadowsharpness);
            }
...
// Calcul des composantes de la lumi�re ambiante
if sl_tod_active
{
...
// R�initialisation des listes des sprites affich�es dans le buffer
